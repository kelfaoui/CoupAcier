...
function ProductDashboard() {
  ...
  const submit = (e) => {
    if (id) {
      ...
      axios.put('http://localhost:5000/products', product
      )
        .then(() => {
  ...
  const getProduct = () => {
    axios.get('http://127.0.0.1:5000/products/' + id)
      .then(function (res) {
        ...
        setNomProduit(res.data.data.nomProduit);
        setPrixMetre(res.data.data.prixMetre);
        setDescription(res.data.data.description);
        setHauteur(res.data.data.hauteur);
        setEpaisseur(res.data.data.epaisseur);
        setMarge(res.data.data.marge);
        setMasseLineaire(res.data.data.masseLineaire);
        setTVA(res.data.data.tva);
        setReferenceProduit(res.data.data.referenceProduit);
        setIdCategorie(res.data.data.idCategorie)
  ...
  return (
    <div className="rounded w-4/5 p-5 flex">
      ...
      {id == null ? <h2 class="text-2xl font-bold mb-4">Nouvel article</h2> : <h2 class="text-2xl font-bold mb-4">Modifier article</h2>}
          ...
          <div class="grid grid-cols-2 gap-10">
            <div>
              ...
              <div class="mb-5">
                ...
                <textarea id="description" onChange={changeDescription} class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" rows="3" placeholder="Saisir la description produit" required>
                  {description}
                </textarea>
              </div>
              <div class="mb-5">
